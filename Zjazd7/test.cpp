...
TEST_CASE("Basic entity operations", "[Car][constructors]") { // [Car][constructor of Car] present
  ...
  SECTION("Have access to private Car fields through getters.") {
    ...
    REQUIRE(car.getEngineDisplacement() == 1300.00);
  }
  ...
  SECTION("Have access to private Car fields through setters.") {
    ...
    car.setEngineDisplacement(1200.00);
    REQUIRE(car.getEngineDisplacement() == 1200.00);
  }
}
...
TEST_CASE("Basic database object operations", "[DataBase][constructors]") { // [db_class][constructor of db_class] present
  ...
  SECTION ("getCarByParam(id,model,brand,engineDisplacement) method is present and Throw: 'Car Not Found'.") {
    DataBase db;
    CHECK_THROWS(db.getCarsByParam(3));
    CHECK_THROWS_WITH(db.getCarsByParam(3), "Car Not Found");
  }

  SECTION ("getCarByParam(id,model,brand,engineDisplacement) Throw: 'Give at last one param' when giving 0 params.") {
    DataBase db;
    CHECK_THROWS_WITH(db.getCarsByParam(), "Give at last one param");
  }

  SECTION ("getCarByParam(id,model,brand,engineDisplacement) returns a List of cars when find SUCCEED.") {
    DataBase db;
  ...
  SECTION ("getCarByParam(model,brand,engineDisplacement) returns a List of cars when find SUCCEED.") {
    DataBase db;
    Car car1("500", "Fiat", 648.00);
  ...
  SECTION ("getCarByParam(brand,engineDisplacement) returns a List of cars when find SUCCEED.") {
    DataBase db;
    Car car1("500", "Fiat", 648.00);
  ...
  SECTION ("getCarByParam(engineDisplacement) returns a List of cars when find SUCCEED.") {
    DataBase db;
    Car car1("500", "Fiat", 648.00);
